...
-- Catch unrecognized special forms
...
numericBinop op singleVale@[_] = throwError $ NumArgs 2 singleVale
numericBinop op params = do
    numbers <- mapM unpackNum params
    return $ Number $ foldl1 op numbers
...
-- eqv function: checks if two LispVal values are equivalent
...
-- Unpacker type to generalize equality checking for different LispVal types
data Unpacker = forall a. Eq a => AnyUnpacker (LispVal -> ThrowsError a)

-- Helper function to unpack and compare two LispVal values
...
-- General equality function: checks equivalence of two LispVal values
equal :: [LispVal] -> ThrowsError LispVal
equal [arg1, arg2] = do
    -- Try primitive equality (number, string, bool)
    primitiveEquals <- liftM or $ mapM (unpackEquals arg1 arg2)
        [ AnyUnpacker unpackNum
        , AnyUnpacker unpackStr
        , AnyUnpacker unpackBool
        ]
    -- Fall back to eqv for compound structures
